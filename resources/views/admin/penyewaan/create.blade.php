...
<div class="max-w-5xl mx-auto">
    <div class="bg-zinc-900 border border-zinc-800 rounded-2xl shadow-lg overflow-hidden">
        <div class="border-b border-zinc-800 px-6 py-4">
            <h2 class="text-2xl font-bold text-white">Tambah Penyewaan Baru</h2>
            <p class="text-sm text-zinc-400">Masukkan data penyewaan dan pilih kendaraan yang tersedia.</p>
        </div>
            ...
            <form action="{{ route('admin.penyewaan.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal_sewa" value="{{ old('tanggal_sewa') }}" class="w-full rounded-lg bg-zinc-800 border border-zinc-700 text-white px-4 py-3" required>
                    </div>

                    <div>
                        <label class="block text-sm mb-2 text-zinc-300">Lama Sewa (hari)</label>
                        <input type="number" name="lama_sewa" min="1" value="{{ old('lama_sewa', 1) }}" class="w-full rounded-lg bg-zinc-800 border border-zinc-700 text-white px-4 py-3" required>
                    </div>
