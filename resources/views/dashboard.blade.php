<x-app-layout class="font-sans antialiased bg-zinc-950 text-zinc-100 min-h-screen relative overflow-x-hidden selection:bg-amber-500 selection:text-black">
    ...
    <div class="min-h-screen bg-zinc-950 py-8">
        <div class="px-4 sm:px-6 lg:px-7 max-w-7xl mx-auto space-y-5">
            ...
            @if(session('success'))
                <div class="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-sm font-medium">
                    {{ session('success') }}
                </div>
            @endif

            @if(session('info'))
                <div class="p-4 rounded-2xl bg-sky-500/10 border border-sky-500/20 text-sky-400 text-sm font-medium">
                    {{ session('info') }}
                </div>
            @endif

            @if(session('error'))
                <div class="p-4 rounded-2xl bg-red-500/10 border border-red-500/20 text-red-400 text-sm font-medium">
                    {{ session('error') }}
                </div>
            @endif
            ...
            <div class="p-6 bg-gradient-to-r from-amber-500/10 via-amber-600/5 to-transparent border border-amber-500/20 rounded-3xl relative overflow-hidden backdrop-blur-md">
                ...
                <div class="max-w-2xl">
                    ...
                    <p class="text-sm text-zinc-300 leading-relaxed">
                        Mau jalan-jalan tetapi tidak punya kendaraan? Temukan kendaraan terbaik mulai dari motor atau mobil terbaik buat kenyamanan keluarga anda.
                    </p>
                    <div class="mt-4 flex gap-3">
                        <a href="{{ route('user.kendaraan.index') }}" class="px-4 py-2 bg-amber-500 text-black hover:bg-amber-400 text-xs font-bold rounded-xl transition-all shadow-[0_4px_15px_rgba(245,158,11,0.25)] active:scale-95">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div class="p-6 bg-zinc-900/60 border border-zinc-800 rounded-2xl relative overflow-hidden group hover:border-zinc-700/60 transition-all duration-300">
                    ...
                    <div class="mt-4 flex items-baseline gap-2">
                        <span class="text-3xl font-bold text-white tracking-tight">{{ $sewaAktif->count() }}</span>
                        <span class="text-xs text-zinc-500">Unit Kendaraan</span>
                    </div>
                    <p class="text-xs text-amber-500 mt-2 font-medium">
                        {{ $sewaAktif->count() > 0 ? 'Sedang berjalan' : 'Belum ada sewa aktif' }}
                    </p>
                </div>
                ...
                <div class="p-6 bg-zinc-900/60 border border-zinc-800 rounded-2xl relative overflow-hidden group hover:border-zinc-700/60 transition-all duration-300">
                    ...
                    <div class="mt-4 flex items-baseline gap-2">
                        <span class="text-3xl font-bold text-white tracking-tight">{{ $totalRiwayat }}</span>
                        <span class="text-xs text-zinc-500">Penyewaan</span>
                    </div>
                    <p class="text-xs text-zinc-400 mt-2">Sepanjang waktu</p>
                </div>

                <div class="p-6 bg-zinc-900/60 border border-zinc-800 rounded-2xl relative overflow-hidden group hover:border-zinc-700/60 transition-all duration-300">
                    <span class="text-xs font-semibold uppercase tracking-wider text-zinc-400">Menunggu Pembayaran</span>
                    <div class="mt-4 flex items-baseline gap-2">
                        <span class="text-3xl font-bold {{ $tagihanPending->count() > 0 ? 'text-red-500' : 'text-white' }} tracking-tight">{{ $tagihanPending->count() }}</span>
                        <span class="text-xs text-zinc-500">Tagihan</span>
                    </div>
                    @if($tagihanPending->count() > 0)
                        <p class="text-xs text-red-400 mt-2 font-semibold">
                            Rp {{ number_format($tagihanPending->sum('amount'), 0, ',', '.') }} belum dibayar
                        </p>
                    @else
                ...
                <div class="p-6 bg-zinc-900/60 border border-zinc-800 rounded-2xl relative overflow-hidden group hover:border-zinc-700/60 transition-all duration-300">
                    <span class="text-xs font-semibold uppercase tracking-wider text-zinc-400">Total Pengeluaran</span>
                    <div class="mt-4 flex items-baseline gap-2">
                        <span class="text-2xl font-bold text-white tracking-tight">Rp {{ number_format($totalPengeluaran, 0, ',', '.') }}</span>
                    </div>
                    <p class="text-xs text-zinc-500 mt-2">Dari pembayaran lunas</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
                ...
                <div class="lg:col-span-7 space-y-6">
                    ...
                    {{-- Sewa Aktif --}}
                    <div class="bg-zinc-900/40 border border-zinc-800 rounded-3xl p-6 relative overflow-hidden backdrop-blur-md">
                        ...
                        @forelse($sewaAktif as $sewa)
                            <div class="p-4 mb-4 last:mb-0 bg-zinc-950/60 border border-zinc-800 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                <div>
                                    <h4 class="text-sm font-bold text-white">{{ $sewa->kendaraan->nama ?? 'Kendaraan tidak ditemukan' }}</h4>
                                    <p class="text-xs text-zinc-500 mt-1">
                                        {{ \Carbon\Carbon::parse($sewa->tanggal_sewa)->translatedFormat('d M Y') }}
                                        &mdash;
                                        {{ \Carbon\Carbon::parse($sewa->tanggal_sewa)->addDays($sewa->lama_sewa - 1)->translatedFormat('d M Y') }}
                                        ({{ $sewa->lama_sewa }} hari)
                                    </p>
                                ...
                                <a href="{{ route('user.penyewaan.show', $sewa->id) }}" class="px-4 py-2 bg-amber-500/10 border border-amber-500/20 text-amber-500 hover:bg-amber-500 hover:text-black rounded-xl text-xs font-bold transition-all text-center">
                                    Lihat Detail
                                </a>
                            </div>
                        @empty
                    ...
                    <div class="bg-zinc-900/40 border border-zinc-800 rounded-3xl p-6 relative overflow-hidden backdrop-blur-md">
                        <div class="flex items-center justify-between border-b border-zinc-800 pb-4 mb-6">
                            <div>
                                <h3 class="text-lg font-bold text-white">Riwayat Penyewaan Terbaru</h3>
                                <p class="text-xs text-zinc-500">5 transaksi penyewaan terakhir Anda.</p>
                            </div>
                        ...
                        @forelse($riwayatTerbaru as $riwayat)
                            @php
                                $statusColor = match($riwayat->status) {
                                    'Disetujui' => 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20',
                                    'Pending' => 'text-amber-400 bg-amber-500/10 border-amber-500/20',
                                    'Ditolak' => 'text-red-400 bg-red-500/10 border-red-500/20',
                                    'Selesai' => 'text-zinc-400 bg-zinc-500/10 border-zinc-500/20',
                                    default => 'text-zinc-400 bg-zinc-500/10 border-zinc-500/20',
                                };
                            @endphp
                            <div class="flex items-center justify-between py-3 {{ !$loop->last ? 'border-b border-zinc-800/60' : '' }}">
                                <div>
                                    <p class="text-sm font-semibold text-white">{{ $riwayat->kendaraan->nama ?? '-' }}</p>
                                    <p class="text-xs text-zinc-500">{{ \Carbon\Carbon::parse($riwayat->tanggal_sewa)->translatedFormat('d M Y') }} &middot; {{ $riwayat->lama_sewa }} hari</p>
                                </div>
                        ...
                        @empty
                            <p class="text-xs text-zinc-500 py-6 text-center">Belum ada riwayat penyewaan.</p>
                        @endforelse
                    </div>
                </div>
                ...
                <div class="lg:col-span-5 space-y-6">
                    <div class="bg-zinc-900/40 border border-zinc-800 rounded-3xl p-6 shadow-2xl relative overflow-hidden backdrop-blur-md">
                        <div class="flex items-center justify-between border-b border-zinc-800 pb-3 mb-5">
                            ...
                            <a href="{{ route('user.kendaraan.index') }}" class="text-xs text-amber-500 hover:text-amber-400 font-semibold transition-colors">
                                Lihat Semua
                            </a>
                        </div>

                        <div class="space-y-4">
                            @forelse($rekomendasiKendaraan as $kendaraan)
                                <div class="p-3 bg-zinc-950/60 border border-zinc-800 hover:border-zinc-700 rounded-xl flex items-center justify-between group transition-all duration-300">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h4 class="text-xs font-semibold text-white">{{ $kendaraan->nama }}</h4>
                                            <p class="text-[10px] text-zinc-500">{{ $kendaraan->merk }} &middot; Rp {{ number_format($kendaraan->harga_sewa, 0, ',', '.') }}/hari</p>
                                        </div>
                                    </div>
                                    <a href="{{ route('user.kendaraan.show', $kendaraan->id) }}" class="px-3 py-1.5 bg-amber-500/10 border border-amber-500/20 text-amber-500 hover:bg-amber-500 hover:text-black rounded-lg text-[10px] font-bold transition-all">
                                        Sewa
                                    </a>
                                </div>
                            @empty
                                <p class="text-xs text-zinc-500 py-6 text-center">Belum ada kendaraan tersedia saat ini.</p>
                            @endforelse
                        </div>
                    </div>
